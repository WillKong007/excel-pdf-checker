...
EN_CONFIG = {
    ...
    "force_ocr": True,
    "max_pages": 0,
    "ocr_dpi": 350,                     # Increased to 350 for better OCR
    "parallel_workers": 2,
    "use_dual_engine": True,            # <-- NEW: enable pdfplumber + OCR for English
...
class ExcelPDFAlphanumericChecker:
    ...
    def extract_pdf_content_parallel(self, pdf_path, progress_callback=None):
        ...
        dpi = self.config.get("ocr_dpi", 350)        # updated to 350
        ocr_config = '--psm 6'
    ...
    # --- UPDATED: Dual engine extraction (pdfplumber + OCR) ---
    def extract_pdf_content(self, pdf_path, progress_callback=None):
        ...
        if self.config.get("use_dual_engine", False):
            ...
            # 2. OCR (parallel) – we need a separate progress callback for OCR
            #    but we'll just run it without page progress to avoid double counting.
    ...
    # --- UPDATED: compare_content with hyphenated fallback ---
    def compare_content(self, excel_text, pdf_text, cell_map):
        ...
        # Helper for hyphenated tokens
        def is_hyphenated_present(token, pdf_clean):
            # Split by any hyphen-like character
        ...
        for token in missing_global:
            ...
            if token_clean in pdf_clean or safe_normalize(token_clean) in safe_normalize(pdf_clean):
                continue
            # 2. Hyphenated fallback (if token contains a hyphen)
        ...
        for (sheet, ref), val in cell_map.items():
            ...
            if ratio < threshold:
                ...
                # Apply universal filter at cell level
                cell_missing_clean = {
                    ...
                    and not is_hyphenated_present(t, pdf_clean)   # also check hyphenated here
                }

                # ---- KEY FIX: For cells with numbers, only report missing numbers ----
                if has_numbers:
...
# ------------------------------------------------------------
# 2. CHINESE CHECKER CONFIG & CLASS (unchanged)
# ------------------------------------------------------------
ZH_CONFIG = {
