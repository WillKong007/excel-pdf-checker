...
class ExcelPDFAlphanumericChecker:
    ...
    def compare_content(self, excel_text, pdf_text, cell_map):
        ...
        # --- UNIVERSAL FILTER with SAFE NUMERIC NORMALISATION ---
        ...
        for token in missing_global:
            ...
            # Check raw AND normalised (only for numeric tokens)
        ...
        for (sheet, ref), val in cell_map.items():
            ...
            if ratio < threshold:
                ...
                # Apply the same filter at cell level
                cell_missing_clean = {
...
# ------------------------------------------------------------
# 2. CHINESE CHECKER CONFIG & CLASS
# ------------------------------------------------------------
ZH_CONFIG = {
