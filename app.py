...
class ExcelPDFAlphanumericChecker:
    ...
    # --- SIMPLIFIED compare_content: ONLY SUBSTRING CHECK ---
    def compare_content(self, excel_text, pdf_text, cell_map):
        ...
        # Prepare cleaned PDF for substring check
        ...
        for (sheet, ref), val in cell_map.items():
            ...
            if cell_clean in pdf_clean_sub:
                ...
                # Missing or truncated – flag immediately
                missing_cells[(sheet, ref)].append(f"TRUNCATED/MISSING: ...{cell_norm[-200:]}")
    ...
    def compare_content(self, excel_text, pdf_text, cell_map):
        ...
        for (sheet, ref), val in cell_map.items():
            ...
            if cell_clean in pdf_chars_stream:
                ...
                # Missing or truncated – flag immediately
                tail = val[-200:] if len(val) > 200 else val
                missing_cells[(sheet, ref)].append(f"TRUNCATED/MISSING: ...{tail}")
                continue  # no further checks
...
if st.button("🔍 Check for Missing Content"):
    ...
    if not results:
        ...
        for lang, data in results.items():
            ...
            cells_with_issues = len(stats['missing_cells'])
