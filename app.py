...
# ------------------------------------------------------------
# 5. STREAMLIT UI – Fixed display for missing and truncated cells
# ------------------------------------------------------------
...
if st.button("🔍 Check for Missing Content"):
    ...
    if not results:
        ...
        for lang, data in results.items():
            ...
            cells_with_truncated = len(stats.get('truncated_cells', {}))
            cells_with_issues = cells_with_missing + cells_with_truncated
            ...
            elif stats.get('truncated_cells'):
                # Only truncated cells, no missing
                st.info(f"No missing {display_name} content, but there are truncations.")
            else:
                st.info(f"🎉 No missing or truncated {display_name} content detected!")

            # ---- Show truncated cells for Chinese ----
            if stats.get('truncated_cells'):
                st.subheader(f"🚨 {display_name} Truncated Cells")
                trunc_rows = []
                for cell, msg in stats['truncated_cells'].items():
                    trunc_rows.append({"Cell": cell, "Issue": msg})
                st.dataframe(pd.DataFrame(trunc_rows), use_container_width=True, hide_index=True)

            # ---- Side-by-Side Preview for missing cells ----
            if stats['missing_cells']:
