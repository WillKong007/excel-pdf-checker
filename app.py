...
class ExcelPDFAlphanumericChecker:
    ...
    def compare_content(self, excel_text, pdf_text, cell_map):
        ...
        for (sheet, ref), val in cell_map.items():
            ...
            # ---- LAYER 1: CELL-LEVEL SEQUENCE CHECK ----
            ...
            # ---- LAYER 2: PREFIX/SUFFIX CHECK (distance‑aware) ----
            if len(tokens_in_cell) > 10:
                prefix = cell_clean[:100]
                suffix = cell_clean[-100:]
                # Find prefix position in cleaned PDF
                prefix_pos = pdf_clean_sub.find(prefix)
                if prefix_pos != -1:
                    # Look within 2000 characters after the prefix
                    window = pdf_clean_sub[prefix_pos:prefix_pos + 2000]
                    if suffix not in window:
                        missing_cells[(sheet, ref)].append(f"TRUNCATED: ...{cell_norm[-200:]}")
                        continue
                else:
                    # Prefix not found – likely severely truncated
                    missing_cells[(sheet, ref)].append(f"MISSING PREFIX: {prefix[:50]}")
                    continue
...
# ------------------------------------------------------------
# 2. CHINESE CHECKER (WITH DISTANCE‑AWARE SUFFIX & BULLET COUNT)
# ------------------------------------------------------------
ZH_CONFIG = {
...
class ExcelPDFChineseChecker:
    ...
    def compare_content(self, excel_text, pdf_text, cell_map):
        ...
        # ---- PDF character stream for substring/suffix checks ----
        ...
        for (sheet, ref), val in cell_map.items():
            ...
            cell_clean = "".join(chars_in_cell)  # full cleaned cell

            # ---- LAYER 1: SUBSTRING CHECK ----
            if cell_clean in pdf_chars_stream:
                continue

            # ---- LAYER 2: DISTANCE‑AWARE PREFIX/SUFFIX CHECK ----
            if len(chars_in_cell) > 10:
                prefix = cell_clean[:50]
                suffix = cell_clean[-50:]
                prefix_pos = pdf_chars_stream.find(prefix)
                if prefix_pos != -1:
                    # Look within the next 2000 characters (roughly one page)
                    window = pdf_chars_stream[prefix_pos:prefix_pos + 2000]
                    if suffix not in window:
                        truncated_cells[f"{sheet}!{ref}"] = f"結尾被裁切（缺少後綴：{suffix}）"
                        continue
                else:
                    # Prefix not found – severely truncated
                    truncated_cells[f"{sheet}!{ref}"] = f"開頭被裁切（缺少前綴：{prefix}）"
                    continue

            # ---- LAYER 3: STRUCTURAL BULLET‑POINT COUNT (FALLBACK) ----
            # This catches cases where the suffix exists elsewhere but is missing from this cell.
            excel_bullets = re.findall(r'[-\•\●\*]\s*([\u4e00-\u9fff]+)', val)
            if len(excel_bullets) > 1:
                pdf_bullets = re.findall(r'[-\•\●\*]\s*([\u4e00-\u9fff]+)', pdf_text)
                if len(pdf_bullets) < len(excel_bullets):
                    missing_bullets = excel_bullets[len(pdf_bullets):]
                    missing_cells[(sheet, ref)].append(f"Missing {len(missing_bullets)} bullet points: {'、'.join(missing_bullets[:5])}")
                    continue

            # ---- LAYER 4: RATIO CHECK (LAST RESORT) ----
