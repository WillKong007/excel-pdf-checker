...
# ------------------------------------------------------------
# 0.1 WORKER FUNCTION FOR PARALLEL OCR (standalone to avoid pickling errors)
# ------------------------------------------------------------
...
EN_CONFIG = {
    ...
    "ocr_psm": 6,                     # default, change to 11 for borderless tables
...
ZH_CONFIG = {
    ...
    "ocr_psm_zh": 6,                  # default, change to 11 for borderless tables
...
if st.button("🔍 Check for Missing Content"):
    ...
    if not results:
        ...
        for lang, data in results.items():
            ...
            # Combine missing and truncated cells for the main metric
            ...
            # ---- Build a unified table for Sheet, Cell, Missing Words, Count ----
            # For missing tokens (English) or missing characters (Chinese)
            if stats['missing_cells'] or stats.get('truncated_cells'):
                ...
                # Add truncated cells (Chinese)
                for cell, msg in stats.get('truncated_cells', {}).items():
                    # Parse the message to extract missing part and count
                    # Example message: "結尾被裁切（缺少後綴：鞣製和修整）"
                    # Extract the part after "缺少後綴：" or fallback to full message
                    missing_part = msg
                    if "缺少後綴：" in msg:
                        ...
                        # fallback: extract any text after colon
                        if "：" in msg:
                            missing_part = msg.split("：")[1]
                    # Split sheet and cell from "Sheet!Cell" format
                    if "!" in cell:
            ...
            # ---- Side-by-Side Preview for missing cells (only if there are missing tokens) ----
