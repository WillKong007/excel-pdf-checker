...
EN_CONFIG = {
    ...
    "ocr_psm": 6,
    "use_visible_text_check": False,   # <-- NEW: toggle to enable visible-text check
...
class ExcelPDFAlphanumericChecker:
    def __init__(self, config=None):
        ...
        self.code_map = {}   # NEW: store codes per cell
    ...
    # --- UPDATED extract_excel_content to also capture code from column A ---
    ...
    # --- NEW: extract visible text from a specific cell by its code ---
    def extract_visible_cell_text_by_code(self, pdf_path, code, column_index=1):
        """
        Extract visible text from a specific cell in a 3‑column table.
        column_index: 0 = Code, 1 = Chinese, 2 = English.
        """
        import pdfplumber
        import re
        with pdfplumber.open(pdf_path) as pdf:
            for page in pdf.pages:
                # Try to detect the table structure (borderless)
                ...
                if not tables:
                    ...
                    if full_text and str(code) in full_text:
                        # Fallback: return entire page text (rough)
                        return full_text
                    continue
                ...
                for table in tables:
                    for row in table.rows:
                        if len(row.cells) <= column_index:
                            continue
                        code_cell = row.cells[0]
                        code_text = page.within_bbox(code_cell).extract_text()
                        if not code_text:
                            continue
                        code_clean = re.sub(r'[^\d]', '', code_text).strip()
                        if str(code) in code_clean or code_clean in str(code):
                            target_cell = row.cells[column_index]
                            visible_text = page.within_bbox(target_cell).extract_text()
                            if visible_text:
    ...
    def compare_content(self, excel_text, pdf_text, cell_map):
        ...
        for (sheet, ref), val in cell_map.items():
            ...
            # ---- LAYER 2: SUFFIX CHECK (for long cells) ----
            if len(tokens_in_cell) > 10:
                ...
                if prefix in pdf_clean_sub and suffix not in pdf_clean_sub:
                    missing_cells[(sheet, ref)].append(f"TRUNCATED: ...{cell_norm[-200:]}")
                    continue
            ...
            # ---- NEW: VISIBLE TEXT LENGTH CHECK (after all other checks) ----
            if self.config.get("use_visible_text_check", False):
                if len(val) > 200:  # only for long cells
                    # Get the code for this cell from the code_map
                    code = self.code_map.get((sheet, ref))
                    if code:
                        visible_text = self.extract_visible_cell_text_by_code(
                            self.current_pdf_path, code, column_index=2  # column 2 = English description
                        )
                        if visible_text is None:
                            # Code not found – skip
                            continue
                        # Count tokens
                        visible_tokens = self.tokenize_english(visible_text)
                        cell_tokens_count = len(tokens_in_cell)
                        if visible_tokens:
                            visible_len = len(visible_tokens)
                            ratio_vis = visible_len / cell_tokens_count
                            # Dynamic threshold
                            if cell_tokens_count < 10:
                                thresh = 0.90
                            elif cell_tokens_count < 50:
                                thresh = 0.80
                            else:
                                thresh = 0.70
                            if ratio_vis < thresh:
                                missing_cells[(sheet, ref)].append(
                                    f"VISUAL TRUNCATION: only {visible_len} tokens visible (expected {cell_tokens_count})"
                                )
                        else:
                            # No visible text at all – flag
                            missing_cells[(sheet, ref)].append(
                                f"VISUAL TRUNCATION: no visible text found"
                            )
    ...
    def check_pair(self, excel_path, pdf_path, progress_callback=None):
        ...
        self.current_pdf_path = pdf_path  # store for visible-text check
...
# ------------------------------------------------------------
# 2. CHINESE CHECKER (with visible-text check)
# ------------------------------------------------------------
ZH_CONFIG = {
    ...
    "ocr_psm_zh": 6,
    "use_visible_text_check": False,   # <-- NEW toggle
...
class ExcelPDFChineseChecker:
    ...
    # --- UPDATED extract_excel_content to capture codes ---
    def extract_excel_content(self, excel_path):
        wb = load_workbook(excel_path, data_only=True)
        all_text = []
        cell_map = {}
        ...
        for sheet_name in wb.sheetnames:
            ...
            # First pass: codes from column A
            for row in range(min_row, max_row + 1):
                if self.config["ignore_hidden"] and ws.row_dimensions[row].hidden:
                    continue
                code_cell = ws.cell(row=row, column=1)
    ...
    def extract_pdf_content(self, pdf_path, progress_callback=None):
        ...
        try:
            ...
            for idx, img in enumerate(images):
                ...
                ocr_text = pytesseract.image_to_string(thresh, lang='chi_sim', config=f'--psm {psm}')
                if ocr_text:
    ...
    # --- NEW: extract visible text by code (same as English checker) ---
    def extract_visible_cell_text_by_code(self, pdf_path, code, column_index=1):
        import pdfplumber
        import re
        with pdfplumber.open(pdf_path) as pdf:
    ...
    # --- UPDATED compare_content to include visible-text check ---
    def compare_content(self, excel_text, pdf_text, cell_map):
        ...
        for (sheet, ref), val in cell_map.items():
            ...
            # ---- RUN THE COMPREHENSIVE CHECK (covers substring, N‑gram, suffix, etc.) ----
            is_present, missing_part = self.check_cell_presence(val, pdf_text)

            if not is_present:
                if any(key in missing_part for key in ["遺漏字元", "疑似截斷", "結尾被裁切", "遺漏段落"]):
                    truncated_cells[f"{sheet}!{ref}"] = missing_part
                else:
                    # fallback ratio
                    present = chars_in_cell & pdf_tokens
                    ratio = len(present) / len(chars_in_cell)
                    if ratio < self.config.get("cell_presence_threshold", 0.95):
                        cell_missing = chars_in_cell - pdf_tokens
                        if cell_missing:
                            missing_cells[(sheet, ref)].extend(list(cell_missing))
            else:
                # ---- NEW: VISIBLE TEXT LENGTH CHECK (only if cell is fully present in digital layer) ----
                if self.config.get("use_visible_text_check", False):
                    if len(val) > 200:  # only for long cells
                        code = self.code_map.get((sheet, ref))
                        if code:
                            visible_text = self.extract_visible_cell_text_by_code(
                                self.current_pdf_path, code, column_index=1  # Chinese description column
                            )
                            if visible_text is None:
                                continue
                            visible_chars = self.tokenize_chinese(visible_text)
                            if visible_chars:
                                visible_len = len(visible_chars)
                                cell_len = len(chars_in_cell)
                                ratio_vis = visible_len / cell_len if cell_len > 0 else 0
                                # Dynamic threshold
                                if cell_len < 10:
                                    thresh = 0.90
                                elif cell_len < 50:
                                    ...
                                    thresh = 0.70
                                if ratio_vis < thresh:
                                    # Flag as visual truncation
                                    truncated_cells[f"{sheet}!{ref}"] = (
                                        f"視覺上只顯示了 {visible_len} 個字元（預期 {cell_len} 個）"
                                    )
                            else:
                                # No visible Chinese text – flag
                                truncated_cells[f"{sheet}!{ref}"] = "視覺上完全沒有中文字元"
...
# NEW: Checkbox to enable visible-text check
with st.expander("⚙️ Advanced Detection Settings"):
    use_visible_check = st.checkbox("Enable visible‑text truncation detection (experimental)", value=False)
    if use_visible_check:
        EN_CONFIG["use_visible_text_check"] = True
        ZH_CONFIG["use_visible_text_check"] = True
    else:
        EN_CONFIG["use_visible_text_check"] = False
        ZH_CONFIG["use_visible_text_check"] = False
