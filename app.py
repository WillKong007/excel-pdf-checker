...
EN_CONFIG = {
    ...
    "ocr_dpi": 350,
    "parallel_workers": 2,
    "use_dual_engine": True,            # pdfplumber + OCR
    "use_substring_check": True,        # <-- NEW: toggle substring matching
...
class ExcelPDFChineseChecker:
    ...
    使用：整格字串比對、唯一字元比例、N-gram (嚴格) 及 結尾後綴檢查。
    """
    ...
    # ---- 2. 整格子字串比對 (快速檢查) ----
    # 如果整個儲存格的文字（去除空格後）直接出現在 PDF 中，代表完整。
    cell_clean = "".join(cell_chars)
    if cell_clean in pdf_chars_stream:
        return True, ""

    # ---- 3. 唯一字元比例檢查 ----
    # 即使遺漏的字元出現在其他儲存格，只要這個儲存格少太多字元，就會觸發。
    ...
    # ---- 5. 結尾後綴檢查 (解決你的主要問題) ----
    # 檢查儲存格的最後 5 個字元。如果結尾被裁掉，這個檢查一定會失敗。
