...
# ------------------------------------------------------------
# 0. HELPER: SAFE NUMERIC NORMALISATION (fixes OCR number errors)
# ------------------------------------------------------------
def normalize_ocr(text):
    """Replace common OCR misreads with their correct numeric equivalents."""
    mapping = {
        'O': '0', 'o': '0', 'Q': '0', 'q': '0',
        'I': '1', 'l': '1', '|': '1', '!': '1',
        'S': '5', 's': '5',
...
def safe_normalize(token):
    """Only normalise tokens that contain at least one digit (safe for English words)."""
...
class ExcelPDFAlphanumericChecker:
    ...
    def _process_single_page(self, pdf_path, page_num, dpi, ocr_config):
        try:
            from pdf2image import convert_from_path
            import pytesseract
            import cv2
            ...
            # Use adaptive threshold for better OCR
    ...
    def extract_pdf_content(self, pdf_path, progress_callback=None):
        ...
        # If dual engine is enabled, run both pdfplumber AND parallel OCR
        if self.config.get("use_dual_engine", False):
            # 1. pdfplumber (fast, digital text)
            try:
                with pdfplumber.open(pdf_path) as pdf:
                    pages = pdf.pages
                    if max_pages > 0:
            ...
            # 2. OCR (parallel) – we run it without page progress to avoid double counting
        ...
        # --- Original logic (if dual engine disabled) ---
    ...
    # --- UPDATED: compare_content with substring check, hyphenated fallback, numeric filter ---
    def compare_content(self, excel_text, pdf_text, cell_map):
        ...
        # --- UNIVERSAL FILTER with SAFE NUMERIC NORMALISATION and HYPHENATED FALLBACK ---
        ...
        # Prepare cleaned PDF for substring check (once per call)
        ...
        for (sheet, ref), val in cell_map.items():
            ...
            # --- SUBSTRING CHECK (optional pre-filter) ---
            ...
            # ---- check if this cell contains any numeric token ----
            ...
            if ratio < threshold:
                ...
                # ---- For cells with numbers, only report missing numbers ----
                if has_numbers:
...
# ------------------------------------------------------------
# 2. CHINESE CHECKER CONFIG & CLASS (with line‑by‑line check)
# ------------------------------------------------------------
ZH_CONFIG = {
...
class ExcelPDFChineseChecker:
    ...
    # --- UPDATED check_cell_presence with line‑by‑line bullet‑point check ---
    def check_cell_presence(self, cell_text, pdf_text):
        """
        檢測中文儲存格的內容是否被截斷（尤其是長列表）。
        使用：整格字串比對、唯一字元比例、N-gram (嚴格)、結尾後綴檢查、以及逐行比對。
        """
        ...
        # ---- 6. 逐行比對 (NEW – THE KEY FIX) ----
        # Split the cell text by newlines and bullet points
        lines = re.split(r'[\n\r]+', cell_text)  # split by newline
        # Further split by bullet symbols like '-', '•', '●'
        bullet_lines = []
        for line in lines:
            # Remove leading/trailing whitespace and bullet symbols
            line_clean = re.sub(r'^[-\•\●\*]\s*', '', line).strip()
            if line_clean:
                bullet_lines.append(line_clean)

        # For each bullet line, check if it appears as a substring in the PDF
        # (using the cleaned version, but we'll keep the original for matching)
        for line in bullet_lines:
            # Clean the line for substring check (remove spaces/punctuation)
            ...
            if line_clean_str not in pdf_chars_stream:
                # The line is missing – flag the cell
                return False, f"遺漏段落：{line[:30]}..."

        # ---- 若所有檢查都通過，表示儲存格內容完整 ----
        return True, ""

    # --- UPDATED compare_content with RATIO CHECK ---
    def compare_content(self, excel_text, pdf_text, cell_map):
        ...
        for (sheet, ref), val in cell_map.items():
            ...
            # Also run the truncation check (end‑suffix, line‑by‑line, etc.)
...
if st.button("🔍 Check for Missing Content"):
    ...
    if not results:
        ...
        for lang, data in results.items():
            ...
            cells_with_missing = len(stats['missing_cells'])
            cell_missing_pct = (cells_with_missing / total_cells * 100) if total_cells > 0 else 0.0

            st.subheader(f"📊 {display_name} Checker Results")
            col1, col2, col3, col4 = st.columns(4)
            col1.metric("Total Cells", total_cells)
            col2.metric("Cells with Missing", cells_with_missing)
            col3.metric("Missing % (Cells)", f"{cell_missing_pct:.2f}%")
